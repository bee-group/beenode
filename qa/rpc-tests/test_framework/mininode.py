...
#
# mininode.py - Beenode P2P network half-a-node
#
# This python code was modified from ArtForz' public domain  half-a-node, as
# found in the mini-node branch of http://github.com/jgarzik/pynode.
#
# NodeConn: an object which manages p2p connectivity to a beenode node
...
def beenodehash(s):
    return beenode_hash.getPoWHash(s)
...
# Objects that map to beenoded objects, which can be serialized/deserialized
...
class CBlockHeader(object):
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(beenodehash(r))
            self.hash = encode(beenodehash(r)[::-1], 'hex_codec').decode('ascii')
...
class msg_headers(object):
    ...
    def deserialize(self, f):
        # comment in beenoded indicates these should be deserialized as blocks
...
class NodeConn(asyncore.dispatcher):
    ...
    def __init__(self, dstaddr, dstport, rpc, callback, net="regtest", services=NODE_NETWORK, send_version=True):
        ...
        print('MiniNode: Connecting to Beenode Node IP # ' + dstaddr + ':' \
            + str(dstport))
