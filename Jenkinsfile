...
// which are part of the Beenode repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="beenode-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /beenode-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "beegroup-beenode/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from beegroup-beenode/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'beegroup-beenode/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from beegroup-beenode/develop"
            }
          }

          if (hasCache) {
            sh "cd /beenode-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /beenode-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /beenode-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /beenode-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /beenode-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /beenode-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /beenode-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
