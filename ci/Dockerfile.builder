...
# beenode_hash
RUN git clone https://github.com/bee-group/beenode_hash
RUN cd beenode_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} beenode
RUN useradd -u ${USER_ID} -g beenode -s /bin/bash -m -d /beenode beenode
...
RUN mkdir /beenode-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /beenode-src && \
