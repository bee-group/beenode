...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(strCommand == NetMsgType::DSACCEPT) {
        ...
        for (const auto& q : vecSpysendQueue) {
            if(q == dsq) {
...
void CPrivateSendServer::CommitFinalTransaction(CConnman& connman)
{
    ...
    if(!CPrivateSend::GetDSTX(hashTx)) {
        CSpysendBroadcastTx dstxNew(finalTransaction, activeMasternode.outpoint, GetAdjustedTime());
        dstxNew.Sign();
...
void CPrivateSendServer::CheckForCompleteQueue(CConnman& connman)
{
    ...
    if(nState == POOL_STATE_QUEUE && IsSessionReady()) {
        ...
        CSpysendQueue dsq(nSessionDenom, nSessionInputCount, activeMasternode.outpoint, GetAdjustedTime(), true);
...
//
bool CPrivateSendServer::AddEntry(const CSpySendEntry& entryNew, PoolMessage& nMessageIDRet)
{
...
bool CPrivateSendServer::IsAcceptableDSA(const CSpysendAccept& dsa, PoolMessage& nMessageIDRet)
{
    if(!fMasternodeMode) return false;
...
bool CPrivateSendServer::CreateNewSession(const CSpysendAccept& dsa, PoolMessage& nMessageIDRet, CConnman& connman)
{
    ...
    if(!fUnitTest) {
        ...
        CSpysendQueue dsq(dsa.nDenom, dsa.nInputCount, activeMasternode.outpoint, GetAdjustedTime(), false);
...
bool CPrivateSendServer::AddUserToExistingSession(const CSpysendAccept& dsa, PoolMessage& nMessageIDRet)
{
...
void ThreadCheckPrivateSendServer(CConnman& connman)
{
    ...
    RenameThread("beenode-ps-server");
