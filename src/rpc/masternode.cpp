...
UniValue masternode(const JSONRPCRequest& request)
{
    ...
    if (strCommand == "count")
    {
        ...
        int nCount;
        masternode_info_t mnInfo;
        mnodeman.GetNextMasternodeInQueueForTmp(0,true, nCount, mnInfo);
    ...
    if (strCommand == "current" || strCommand == "winner")
    {
        ...
        if(!mnodeman.GetNextMasternodeInQueueForTmp(nHeight, true, nCount, mnInfo))
            return "unknown";
...
UniValue masternodelist(const JSONRPCRequest& request)
{
                ...
                strMode != "rank" && strMode != "sentinel" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print Beenode address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
