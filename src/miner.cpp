...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn)
{
...

    {
        ...
        // Update coinbase transaction with additional info about masternode,
        // get some info back to pass to getblocktemplate
        FillBlockPayments(txNew, nHeight, blockReward, blockEvolution, pblock->txoutMasternode, pblock->voutSuperblock);
...
// ***TODO*** ScanHash is not yet used in Beenode
//
...
{
    LogPrintf("BeenodeMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("beenode-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("BeenodeMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("BeenodeMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("BeenodeMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("BeenodeMiner -- runtime error: %s\n", e.what());
        return;
    }
}
