...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total beenode balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized beenode balance of the wallet\n"
...
UniValue debug(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "beenode|privatesend|instantsend|masternode|spork|keepass|mnpayments|gobject )\n"
            "Change debug category on the fly. Specify single category or use comma to specify many.\n"
            "\nExamples:\n"
            + HelpExampleCli("debug", "beenode")
            + HelpExampleRpc("debug", "beenode,net")
        );
...
UniValue spork(const UniValue& params, bool fHelp)
{
    if(params.size() == 1 && params[0].get_str() == "show"){
		...
		if(  (nSporkID == SPORK_19_EVOLUTION_PAYMENTS_ENFORCEMENT) || (nSporkID == SPORK_11_MASTERNODE_ENABLED) )
		{
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"beenodeaddress\"\n"
            "\nReturn information about the given beenode address.\n"
            "\nArguments:\n"
            "1. \"beenodeaddress\"     (string, required) The beenode address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"beenodeaddress\", (string) The beenode address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Beenode address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are beenode addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) beenode address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"beenodeaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"beenodeaddress\"  (string, required) The beenode address to use for the signature.\n"
