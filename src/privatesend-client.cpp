...
bool CPrivateSendClientSession::JoinExistingQueue(CAmount nBalanceNeedsAnonymized, CConnman& connman)
{
    ...
    while (privateSendClient.GetQueueItemAndTry(dsq)) {
        ...
        pendingDsaRequest = CPendingDsaRequest(infoMn.addr, CSpysendAccept(nSessionDenom, txMyCollateral));
...
bool CPrivateSendClientSession::StartNewQueue(CAmount nValueMin, CAmount nBalanceNeedsAnonymized, CConnman& connman)
{
    ...
    if (!pwalletMain->SelectSpyCoins(nValueMin, nBalanceNeedsAnonymized, vecTxIn, nValueInTmp, 0, privateSendClient.nPrivateSendRounds - 1)) {
...
void CPrivateSendClientSession::RelayIn(const CSpySendEntry& entry, CConnman& connman)
{
    if (!infoMixingMasternode.fInfoValid) return;
