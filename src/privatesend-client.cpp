...
bool CPrivateSendClient::SendDenominate(const std::vector<CTxDSIn>& vecTxDSIn, const std::vector<CTxOut>& vecTxOut, CConnman& connman)
{
    ...
    CSpySendEntry entry(vecTxDSIn, vecTxOut, txMyCollateral);
...
bool CPrivateSendClient::JoinExistingQueue(CAmount nBalanceNeedsAnonymized, CConnman& connman)
{
    ...
    for (auto& dsq : vecSpysendQueue) {
        ...
        bool fNewAlgo = infoMn.nProtocolVersion > 70208;
        ...
        pendingDsaRequest = CPendingDsaRequest(infoMn.addr, CSpysendAccept(nSessionDenom, nSessionInputCount, txMyCollateral));
...
void CPrivateSendClient::RelayIn(const CSpySendEntry& entry, CConnman& connman)
{
    if(!infoMixingMasternode.fInfoValid) return;
...
void ThreadCheckPrivateSendClient(CConnman& connman)
{
    ...
    RenameThread("beenode-ps-client");
