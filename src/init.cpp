...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, mempoolrej, net, proxy, prune, http, libevent, tor, zmq, "
                             "beenode (or specifically: privatesend, instantsend, masternode, spork, keepass, mnpayments, gobject)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Beenode specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N BEENODE anonymized (default: %u)"), DEFAULT_PRIVATESEND_AMOUNT));
...
std::string LicenseInfo()
{
    ...
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The BeeGroup developers are EternityGroup"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Beenode Core version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}

/** Initialize Beenode Core.
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Beenode Core is shutting down."));
    ...
    try {
        ...
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Beenode Core is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Beenode Core is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Beenode Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Beenode Core to complete") << "\n";
        ...
        if (GetBoolArg("-upgradewallet", fFirstRun))
        {
			...
			{
                printf( "\n\nnMaxVersion = %d,  pwalletMain->GetVersion() = %d\n\n", nMaxVersion, pwalletMain->GetVersion() );
    ...
    if(fMasterNode) {
        ...
        if(!strMasterNodePrivKey.empty()) {
            if(!spySendSigner.GetKeysFromSecret(strMasterNodePrivKey, activeMasternode.keyMasternode, activeMasternode.pubKeyMasternode))
    ...
    spySendPool.SetMinBlockSpacing(nLiquidityProvider * 15);
    ...
    // ********************************************************* Step 11c: update block tip in Beenode modules

    // force UpdatedBlockTip to initialize pCurrentBlockIndex for DS, MN payments and budgets
    // but don't call it directly to prevent triggering of other listeners like zmq etc.
    // GetMainSignals().UpdatedBlockTip(chainActive.Tip());
    mnodeman.UpdatedBlockTip(chainActive.Tip());
    spySendPool.UpdatedBlockTip(chainActive.Tip());
    mnpayments.UpdatedBlockTip(chainActive.Tip());
    masternodeSync.UpdatedBlockTip(chainActive.Tip());

    // ********************************************************* Step 11d: start beenode-privatesend thread
    ...
    // This code will be disabled for 0.7.1 while a fix is deliberated in #7568
