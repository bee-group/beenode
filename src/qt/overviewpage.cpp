...
public:
    TxViewDelegate(const PlatformStyle *_platformStyle, QObject *parent=nullptr):
        QAbstractItemDelegate(parent), unit(BitcoinUnits::BEENODE),
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    {
        // update the display unit, to not use the default ("BEENODE")
        updateDisplayUnit();
...
{
    QString link="http://www.beenode.org/en";
       QDesktopServices::openUrl(QUrl(link));
}

void OverviewPage::on_pushButton_2_clicked()
{
    QString link="https://bee-group.org/";
       QDesktopServices::openUrl(QUrl(link));
}
