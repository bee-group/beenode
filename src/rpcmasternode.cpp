...
UniValue getpoolinfo(const UniValue& params, bool fHelp)
{
    ...
    if (spySendPool.pSubmittedToMasternode) {
        obj.push_back(Pair("outpoint",      spySendPool.pSubmittedToMasternode->vin.prevout.ToStringShort()));
        obj.push_back(Pair("addr",          spySendPool.pSubmittedToMasternode->addr.ToString()));
    }
        ...
        (strCommand != "start" && strCommand != "start-alias" && strCommand != "start-all" && strCommand != "start-missing" &&
         ...
         strCommand != "connect" && strCommand != "outputs" && strCommand != "status"))
            throw std::runtime_error(
                ...
                "  start        - Start local Hot masternode configured in beenode.conf\n"
