...
const char * const BITCOIN_CONF_FILENAME = "beenode.conf";
const char * const BITCOIN_PID_FILENAME = "beenoded.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            if(ptrCategory->count(string("beenode"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BeenodeCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BeenodeCore
    // Mac: ~/Library/Application Support/BeenodeCore
    // Unix: ~/.beenodecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BeenodeCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BeenodeCore";
#else
    // Unix
    return pathRet / ".beenodecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty beenode.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override beenode.conf
