...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Beenode Core is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Beenode Core is downloading blocks...");
    ...
    result.push_back(Pair("masternode_payments_started", pindexPrev->nHeight + 1 > sporkManager.GetSporkValue(SPORK_11_MASTERNODE_ENABLED)) );
