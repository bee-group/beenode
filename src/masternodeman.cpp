...
/* Only IPv4 masternodes are allowed in 7.1, saving this for later
...
void CMasternodeMan::ProcessMasternodeConnections()
{
    ...
    BOOST_FOREACH(CNode* pnode, vNodes) {
        if(pnode->fMasternode) {
            if(spySendPool.pSubmittedToMasternode != NULL && pnode->addr == spySendPool.pSubmittedToMasternode->addr) continue;
...
void CMasternodeMan::SendVerifyReply(CNode* pnode, CMasternodeVerification& mnv)
{
    ...
    if(!spySendSigner.SignMessage(strMessage, mnv.vchSig1, activeMasternode.keyMasternode)) {
    ...
    if(!spySendSigner.VerifyMessage(activeMasternode.pubKeyMasternode, mnv.vchSig1, strMessage, strError)) {
...

    {
        ...
        while(it != vMasternodes.end()) {
            if((CAddress)it->addr == pnode->addr) {
                if(spySendSigner.VerifyMessage(it->pubKeyMasternode, mnv.vchSig1, strMessage1, strError)) {
                    // found it!
                    prealMasternode = &(*it);
                    ...
                    // ... and sign it
                    if(!spySendSigner.SignMessage(strMessage2, mnv.vchSig2, activeMasternode.keyMasternode)) {
                    ...
                    if(!spySendSigner.VerifyMessage(activeMasternode.pubKeyMasternode, mnv.vchSig2, strMessage2, strError)) {
...
void CMasternodeMan::ProcessVerifyBroadcast(CNode* pnode, const CMasternodeVerification& mnv)
{
...

    {
        ...
        if(spySendSigner.VerifyMessage(pmn1->pubKeyMasternode, mnv.vchSig1, strMessage1, strError)) {
            LogPrintf("MasternodeMan::ProcessVerifyBroadcast -- VerifyMessage() for masternode1 failed, error: %s\n", strError);
            return;
        }

        if(spySendSigner.VerifyMessage(pmn2->pubKeyMasternode, mnv.vchSig2, strMessage2, strError)) {
