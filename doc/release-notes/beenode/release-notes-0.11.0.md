...
Spycoin Core 0.11.0 is forked off the Bitcoin Core 0.9.3. The old spycoin
...
with the introduced changes to the Spycoin Core 0.11.0.


Spycoin Core:

- Rebrand to `Spycoin Core`
...
- Defined BIP32 (HD) address versions to start with `drkp`/`drkv` (`DRKP`/`DRKV`
  for testnet)
- Adapted BIP44 coin type `5` for Spycoin (0x80000005) as defined in SLIP-0044
...
- Add separate spycoin-cli client
...
- Reduced lower spysend limit to 1.5 DRK
...
- Added more detailed Spysend status information to Overview tab
- Added Spysend high precision matching engine
- Added Spysend balance to `getinfo`
...
- Add '-disablewallet' mode to let beenoded run entirely without wallet (when
  built with wallet)
- Update default '-rpcsslciphers' to include TLSv1.2
- make '-logtimestamps' default on and rework help-message
- RPC client option: '-rpcwait', to wait for server start
- Remove '-logtodebugger'
- Allow `-noserver` with beenoded
...
- Fix "Start spycoin on system login"
...
- Show and store message of normal spycoin:URI
- Fix richtext detection hang issue on very old Qt versions
- OS X: Make use of the 10.8+ user notification center to display Growl-like
  notifications
- OS X: Added NSHighResolutionCapable flag to Info.plist for better font
  rendering on Retina displays.
- OS X: Fix spycoin-qt startup crash when clicking dock icon
- Linux: Fix Gnome spycoin: URI handler
