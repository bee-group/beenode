...
- saction for Spysend. These are special transactions that require a signature that only the masternodes can create.
- Spysend now has no fees to track what-so-ever, all that will ever be in Spysend transactions are Spysend denominations.
...
- Split up spysend.cpp into masternode.cpp, activemasternode.cpp and instantx.cpp
- Added modular ProcessMessages for Spysend, Masternodes and InstantX
...
- Fixed all sorts of issues with collateral and Spysend.
...
- Spysend tested with 2.5 DRK successfully and I've set that as the minimum
...
- Spysend UI is disabled for masternodes now and titlebar says "[masternode]"
...
- Updated wording for spysend messages
- Removed disable spysend checkbox from config screen
- Added "-enablespysend" cli option
...
- Fixed issue with denominating small amounts of DRK in large wallet
  (http://jira.spycoin.qa/browse/DRK-46)
...
- Spysend should anonymize very close to the intended amount now
- Added Amount/Rounds to overview screen so you can see current settings
- Overview spysend cache is cleared on settings change (will instantly update)
- Fixed issue with completed amount jumping around
  (http://jira.spycoin.qa/browse/DRK-46)
- Made messages less threatening (http://jira.spycoin.qa/browse/DRK-60)
...
- This update deals with freezing, slow wallets, slow load times and the
  ...
  Spysend process has been reserve keys for every attempt! To rectify this
...
- Easy to read spysend progress bar (Mouse over for very detailed info)
...
http://test.explorer.spycoin.fr/tx/ce0ea2bdf630233955d459489b6f764e0d0bbe9e8a62531dd2a14b455626b59c

- Client now creates fee sizes inputs for use in spysend denomination phases
...
- Client now prioritises sending denominated funds back into Spysend. This will
  improve anonymity and help to respect the "anonymize spycoin" amount in the
  configuration.
- fixed a bug where masternodes send failed transactions
- changed max to 100k in configuration
- added a warning message to startup (delete ~/.spycoin/.agree_to_tou to see it)
- found a bug causing inputs to get locked forever.
- Spysend now checks diskspace before sending anything to a masternode.
...
- Majorly improved the way spysend participants are paired together. It should
  be super fast now.
...
- Spysend now shows address instead of "n/a"
...
- Added GUI configuration for Spysend Rounds, Enable Spysend and Anonymize
  amount of DRK
...
http://test.explorer.spycoin.fr/tx/6de2c5204abdea451da930f61bae0f954eef13188a3a37a572a24c9d92057d5d
...
- Spysend limited to 5000DRK per wallet.dat. Client will warn about this the
  first time it's opened, then disable spysend from then on.
...
- Added a pre-session state where the client will query a random masternode
  and ask if they can perform a merge on N spycoin without giving any other
...
- Found race condition with new blocks and clearing spysend entries that was
...
- Spysend Denominate Outputs are now in a random order:

http://test.explorer.spycoin.fr/tx/072ca56cbf705b87749513a2d2ee02080d506adcf8fe178f6dc2967f0711788e
http://test.explorer.spycoin.fr/tx/32daa8ca46462e7e99f3532251d68a8c3835a080c937bd83b11db74e47b770ff

- Spysend now uses 3 participants instead of two.
- SplitUpMoney can now make collateral inputs when needed
- Transactions now shows spysend transaction types for easier understanding
  of what's going on:
...
- Fixed the money destruction bug, it was caused by "spysend denominate 8000".
  I missed a reference and the client passed an empty address to
  SendMoneyToDestination. rcp spysend source: http://pastebin.com/r14piKuq
...
- Added Spysend detection to the UI
- fixed senttoaddress, it will use all inputs when spysend is disabled now.
  Otherwise it will ONLY use non-denom.
- "spysend addr amount" now returns the hash of the transaction it creates
...
- Removed "spysend denominate", spysend now will figure out the most it can
  denominate. Use "spysend auto" instead.
...
- QT GUI will now ask to unlock the wallet when it detects Spysend wants to do
  something and lock it when it's done again.
- Spysend is turned off by default in the daemon now. In most cases daemons
  won't want to run with anonymity (pools, exchanges, etc), if a user does they
  can override the default setting with -enabledaemonspysend=true
- Fees per round of Spysend are 0.001DRK or $0.00538 at current prices. This
...
- RPC calls are changed a bit:
    ...
    spysend denominate 100 - Will denominate 100DRK
    spysend auto - Will run AutoDenominate
    spysend Xaddr 100 - Will send 100 denominated DRK to Xaddr
...
- Fixed spysend+ rounds function, it was returning bad data in some cases
  causing spysend inputs to never stop being sent through spysend.
...
/spycoin-qt -datadir=/home/user/.spycoin -listen=0 -spysendrounds=8 -usemasternode="192.168.56.102:19999"
/spycoin-qt -datadir=/home/user/.spycoin2 -listen=0 -spysendrounds=8 -usemasternode="192.168.56.102:19999"

and even disable spysend auto-denom if wanted:
/spycoin-qt -datadir=/home/user/.spycoin -listen=0 -spysendrounds=8 -usemasternode="192.168.56.102:19999" -disablespysend=1
...
- Added a smart input splitting method. Place 1000+DRK into a brand new wallet
  and it will be split into many inputs compatible with Spysend
...
- SpySendRounds had a bug that I fixed, should calculate correctly now
...
- New column "Spysend Rounds" in coincontrol to show how secure a given input is
- Fixed a few issues causing spysend to fail. We should see many more spysends
...
- Redid denominations to 1, 10, 100, and 500. Maybe this is too simple, but it
  ...
  again through spysend for the next transactions. We'll see how it works.
- usemasternode option, will override active masternode (only in RC, just for testing)

0.10.7.3 Release notes
-----------------------
...
- Improved SpysendInput add entry verification. Masternodes will now reject
...
- resolves issue with wallet not syncing by adding the capability to retrieve nodes through dnsseed
   (see https://github.com/spycoinproject/spycoin/pull/21)
- Linux 32 and Mac OS X are now officially supported platforms
   (see https://github.com/spycoinproject/spycoin/pull/17)
- improved overall unit test code coverage
   (see https://github.com/spycoinproject/spycoin/pull/13 and https://github.com/spycoinproject/spycoin/pull/15)
- minor documentation updates
   (https://github.com/spycoinproject/spycoin/pull/18)
