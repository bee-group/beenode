...
Beenode Core is a multithreaded application, and deadlocks or other multithreading bugs
...
However, in open source software such as Beenode Core, where everyone uses
...
pay attention to for reviewers of Beenode Core code.

General Beenode Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Beenode Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Beenode Core so that they can be integrated
