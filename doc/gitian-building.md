...
*Setup instructions for a Gitian build of Beenode Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Beenode
...
- [Building Beenode Core](#building-beenode-core)
...
Clone the git repositories for Beenode Core and Gitian.
...
in the Beenode Core repository under 'Fetch and create inputs' to install sources which require
...
To build Beenode Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Beenode Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/beenode/.git/
...
For example:
```bash
URL=https://github.com/crowning-/beenode.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit beenode=${COMMIT} --url beenode=${URL} ../beenode/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit beenode=${COMMIT} --url beenode=${URL} ../beenode/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit beenode=${COMMIT} --url beenode=${URL} ../beenode/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the beenode git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../beenode/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/beenode
SIGPATH=/some/root/path/beenode-detached-sigs

./bin/gbuild --url beenode=${BTCPATH},signature=${SIGPATH} ../beenode/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/beenode-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/beenode-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/beenode-osx-build.assert
```
...
[beenode/gitian.sigs](https://github.com/beenodepay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
